...
public class TeacherController {
    ...
    public record QuizRes(long id, String title, String joinPin, boolean active, Instant createdAt, long schoolClassId) {}

    @GetMapping("/classes/{classId}/quizzes")
    public List<QuizRes> listQuizzes(@PathVariable long classId) {
        return quizRepo.findBySchoolClassId(classId).stream()
                .map(q -> new QuizRes(q.getId(), q.getTitle(), q.getJoinPin(), q.isActive(), q.getCreatedAt(), q.getSchoolClass().getId()))
                .toList();
    }

    @PostMapping("/classes/{classId}/quizzes")
    public QuizRes createQuiz(@PathVariable long classId, @Valid @RequestBody CreateQuizReq req) {
        SchoolClass sc = classRepo.findById(classId).orElseThrow();
        String pin = pinTokenService.newPin(6);
        Quiz saved = quizRepo.save(new Quiz(sc, req.title(), pin));
        return new QuizRes(saved.getId(), saved.getTitle(), saved.getJoinPin(), saved.isActive(), saved.getCreatedAt(), saved.getSchoolClass().getId());
    }
